...
# --- Setup Rubric and Deadline ---
def setup_rubric_and_deadline():
    print("📄 Enter the **FULL PATH** to your rubric PDF file (e.g., /Users/user/Desktop/rubric.pdf)")
    
    # REPLACED: uploaded = files.upload()
    ...
    # Note: We upload the file object immediately before parsing/grading to reduce latency.
    ...
    print("\n✅ Setup complete.")
    print(f"   • Parsed {len(parsed_rubrics)} rubrics.")
    print(f"   • Deadline (UTC): {DEADLINE.strftime('%Y-%m-%d %H:%M:%S %Z')}" if DEADLINE else "   • Deadline: None")
    print(f"   • Max Attempts: {MAX_ATTEMPTS if MAX_ATTEMPTS is not None else 'Unlimited'}")

    return parsed_rubrics, DEADLINE, MAX_ATTEMPTS

# --- Grading and Persistence ---
def run_grading_process(parsed_rubrics, DEADLINE, MAX_ATTEMPTS):
    if not db_client:
    ...
    # The grade_submission function handles the file upload to Gemini internally now.
    ...
    # The total score calculation needs to use the correct denominator, which is 10 points per rubric item.
    total_rubric_items = len(rubric_meta['parsed_rubrics'])
...
if __name__ == "__main__":
    try:
        parsed_rubrics, DEADLINE, MAX_ATTEMPTS = setup_rubric_and_deadline()
        run_grading_process(parsed_rubrics, DEADLINE, MAX_ATTEMPTS)
